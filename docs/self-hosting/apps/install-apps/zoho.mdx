...
<Steps>
    ...
    <Step title="Set the Redirect URL for OAuth">
        ...
        ```
        <freeCal URL>/api/integrations/zohocrm/callback
        ```
        Replace `<freeCal URL>` with your application URL.
    </Step>
    ...
    <Step title="Integration is Ready">
        Your ZohoCRM integration can now be easily added in the freeCal settings.
    </Step>
</Steps>
...
<Steps>
    ...
    <Step title="Create a New Server-based Application">
        ...
        ```
        <freeCal URL>/api/integrations/zohocalendar/callback
        ```
        Replace `<freeCal URL>` with your application URL.
    </Step>
    <Step title="Fill in Client Details">
    ...
    <Step title="Save Credentials in freeCal Admin Panel">
        Copy the Client ID and Client Secret to your app keys in the freeCal admin panel at:
        ```
        <freeCal>/settings/admin/apps
        ```
    </Step>
    <Step title="Enable Multi-DC Option">
...
<Steps>
    ...
    <Step title="Set the Redirect URL for OAuth">
        Set the Redirect URL for OAuth to:
        ```
        <freeCal URL>/api/integrations/zoho-bigin/callback
        ```
        Replace `<freeCal URL>` with your application URL.
    </Step>
    <Step title="Go to the Client Secret tab">
    ...
    <Step title="Complete Zoho Bigin Integration">
        Your Zoho Bigin integration is now ready and can be added from the freeCal app store.
    </Step>
</Steps>
