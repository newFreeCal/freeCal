...
<Steps>
    ...
    <Step title="Add Authorized Redirect URIs">
        ...
        ```
        <freeCal URL>/api/integrations/googlecalendar/callback
        <freeCal URL>/api/auth/callback/google
        ```
        Replace `<freeCal URL>` with the URL where your application runs.
    </Step>
    ...
    <Step title="Set the CAL_COM environment variable">
        ...
        CALCOM_ENV="local"
        ```
        This will allow you to test the Google integration locally. Do it only when you are testing freeCal locally.
    </Step>
</Steps>

### **Adding Google Calendar to freeCal App Store**
