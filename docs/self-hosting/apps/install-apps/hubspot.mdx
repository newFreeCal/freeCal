...
<Steps>
    ...
    <Step title="Set the Redirect URL for OAuth">
        ...
        ```
        <freeCal URL>/api/integrations/hubspot/callback
        ```
        Replace `<freeCal URL>` with the URL where your application is hosted.
    </Step>
    ...
    <Step title="Complete HubSpot Integration">
        You’re all set! Any booking in freeCal will now be created as a meeting in HubSpot for your contacts.
    </Step>
</Steps>
