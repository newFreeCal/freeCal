...
<Steps>
    ...
    <Step title="Add the redirect URL">
        Add the following redirect URL, replacing `<freeCal URL>` with your application's URL:
        ```
        <freeCal URL>/api/integrations/stripepayment/callback
        ```
    </Step>
    ...
    <Step title="Set up a Stripe webhook">
        ...
        ```
        <freeCal URL>/api/integrations/stripepayment/webhook
        ```
