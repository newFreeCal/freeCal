...
There are multiple ways in which you can deploy freeCal, providing support for customers who want to implement freeCal within their existing infrastructure stack. Let's go through them one-by-one. You can find the instructions for deployment in our README file, which is the section you see when you scroll down in our GitHub repository, or if you've got a copy of freeCal downloaded already, you can open the file contained in the downloaded repository called `README.md`.

## Requirements

freeCal runs with pretty minimal hardware requirements by itself. The most intensive part for the software is when you actually build the software, but once it's running it's relatively lightweight.

freeCal works with a very large range of operating systems, as it only requires JavaScript execution to run. **freeCal is known to work well with Windows, Mac, Linux and BSD.** Although they do work well on all of them, for production deployments we would suggest Linux as the ideal platform. Any operating system that runs Node.js should be able to work too, but these are some of the common operating systems that we know work well.

To run freeCal, you need to install a few things. **Node.js, yarn, Git and PostgreSQL**. We use **Prisma** for database maintenance, and is one of the dependencies. We won’t publish installation guides for these as they have their own resources available on the internet. If you're on Linux/BSD, all of these things should be readily available on your package manager. Your best bet is searching for something like **Debian 12 PostgreSQL**, which will give you a guide to installing and configuring PostgreSQL on Debian Linux 12.
...
```
git clone -c core.symlinks=true https://github.com/calcom/freeCal.git
```
</Note>
...
2. Then, go into the directory you just cloned with
    ...
    yarn
    ```
    to install all of the dependencies. Essentially, dependencies are just things that freeCal needs to install to be able to work.
...
At freeCal, the cron jobs are found in the following directory:
```
...
### Vercel
<a href="https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fcalcom%2FfreeCal&env=DATABASE_URL,NEXT_PUBLIC_WEBAPP_URL,NEXTAUTH_URL,NEXTAUTH_SECRET,CRON_API_KEY,CALENDSO_ENCRYPTION_KEY&envDescription=See%20all%20available%20env%20vars&envLink=https%3A%2F%2Fgithub.com%2Fcalcom%2FfreeCal%2Fblob%2Fmain%2F.env.example&project-name=cal&repo-name=freeCal&build-command=cd%20../..%20%26%26%20yarn%20build&root-directory=apps%2Fweb%2F" target="_blank">
  <img src="https://vercel.com/button" noZoom alt="Deploy with Vercel" />
</a>

## Other environments

freeCal effectively is just a Next.js application, so any possible solution you find online pertaining to Next.js applications should work. One example is Netlify, which is pretty similar to Vercel. It says it supports Next.js, so you can deploy freeCal on Netlify. Refer to Netlify's docs on Next.js projects for more info. Another example is on a self hosted instance people may want to configure complex reverse proxies, SSL gateways and all sorts of other stuff. We can't officially support every configuration, but for any edge case where you may want to deploy freeCal with X, just refer to X's docs on Next.js applications and you should be fine.

That's it. Your new self hosted freeCal instance should now be up and running.
