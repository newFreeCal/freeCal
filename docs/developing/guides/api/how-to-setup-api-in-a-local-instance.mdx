...
<Steps>
    <Step title="Clone the freeCal repository">
        Please clone the freeCal repository. You can do it by following the instructions provided in [Installation](/developing/local-development)
    </Step>
    ...
    <Step title="Start development server and create API key">
        Start the freeCal server using `yarn dev` on localhost and create the test API keys by visiting `/settings/developer/api-keys`
    </Step>
