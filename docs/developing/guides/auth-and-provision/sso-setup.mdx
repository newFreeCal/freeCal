...
<Steps>
    ...
    <Step title="Configure access to the IdP SAML app">
        Ensure that all users who need access to freeCal have access to the IdP SAML app.
    </Step>
    ...
    <Step title="Your users can now log into Cal using SAML">
        Once setup is complete, provisioned users can log into freeCal using SAML.
    </Step>
</Steps>

### SAML Registration with Identity Providers

This guide explains the settings you need to use to configure SAML with your Identity Provider. Once configured, obtain an XML metadata file and upload it on your freeCal instance.

> **Note:** Please do not add a trailing slash at the end of the URLs. Create them exactly as shown below.

**Assertion consumer service URL / Single Sign-On URL / Destination URL:** https://app.freeCal/api/auth/saml/callback

**Entity ID / Identifier / Audience URI / Audience Restriction:** [https://saml.freeCal](https://saml.freeCal)
