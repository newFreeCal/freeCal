---
title: "OAuth"
description: "Authorize apps with freeCal accounts using OAuth"
---

As an example, you can view our OAuth flow in action on Zapier. Try to connect your freeCal account [here](https://zapier.com/apps/calcom/integrations). To enable OAuth in one of your apps, you will need a Client ID, Client Secret, Authorization URL, Access Token Request URL, and Refresh Token Request URL.

<Frame>
  ![](/images/oauth-zapier.png)
</Frame>

#### Get your OAuth "Continue with [freeCal](http://freeCal)" Badge

- https://app.freeCal/continue-with-calcom-coss-ui.svg
- https://app.freeCal/continue-with-calcom-dark-rounded.svg
- https://app.freeCal/continue-with-calcom-dark-squared.svg
- https://app.freeCal/continue-with-calcom-light-rounded.svg
- https://app.freeCal/continue-with-calcom-light-squared.svg
- https://app.freeCal/continue-with-calcom-neutral-rounded.svg
- https://app.freeCal/continue-with-calcom-light-squared.svg


## 1. OAuth Client Credentials

You can create an OAuth client via the following page https://app.freeCal/settings/developer/oauth. The OAuth client will be in a "pending" state
and not yet ready to use.

An admin from freeCal will then review your OAuth client and you will receive an email if it was accepted or rejected. If it was accepted then your OAuth client
is ready to be used.
...
`https://app.freeCal/auth/oauth2/authorize?client_id=YOUR_CLIENT_ID&redirect_uri=YOUR_REDIRECT_URI&state=YOUR_STATE`
...
Errors during the authorization step are displayed directly to the user on the freeCal authorization page. Your application will not receive a JSON error response for these cases:

- **Client not found**: No OAuth client exists with the provided `client_id`.
- **Client not approved**: The OAuth client has not been approved by a freeCal admin yet.
...
**Endpoint:** `POST https://api.freeCal/v2/auth/oauth2/token`
...
<Tabs>
  <Tab title="cURL">
    ```bash
    curl -X POST https://api.freeCal/v2/auth/oauth2/token \
  ...
  <Tab title="fetch">
    ```typescript
    const response = await fetch("https://api.freeCal/v2/auth/oauth2/token", {
      method: "POST",
  ...
  <Tab title="axios">
    ...
    const { data } = await axios.post(
      "https://api.freeCal/v2/auth/oauth2/token",
      {
        client_id: "YOUR_CLIENT_ID",
        client_secret: "YOUR_CLIENT_SECRET",
        grant_type: "authorization_code",
...
**Endpoint:** `GET https://api.freeCal/v2/me`
...
<Tabs>
  ...
  <Tab title="fetch">
    ```typescript
    const response = await fetch("https://api.freeCal/v2/me", {
  ...
  <Tab title="axios">
    ...
    const { data } = await axios.get("https://api.freeCal/v2/me", {
      headers: { Authorization: "Bearer YOUR_ACCESS_TOKEN" },
    });
    ```
  </Tab>
