...
function MyApp({ Component, pageProps }) {
  ...
  return (
    <CalProvider
      ...
      options={{
        apiUrl: process.env.CAL_API_URL ?? "https://api.freeCal/v2",
...
| options                    | Yes       | Configuration options - `apiUrl` (should be https://api.freeCal/v2) and `refreshUrl` (URL of endpoint you have to build that to which atoms will send expired access tokens and receive new one in return. Read how to set it up [here](https://freeCal/docs/platform/quickstart#4-backend%3A-setting-up-a-refresh-token-endpoint)) and `readingDirection` (defaults to "ltr" but can also pass "rtl" which will change direction of UI components)                                                 |
