...
### 3. Are a freeCal user and a managed user related?
- No, a freeCal user and a managed user are not related in any way. We can consider them as two separate entities.
- A "managed user" is completely independent of user accounts on freeCal and you don't need to register users on the freeCal web application.
