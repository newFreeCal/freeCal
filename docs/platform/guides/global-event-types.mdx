...
1. You need to have an OAuth client set up, since we'll need the client id and client secret from the OAuth client to be included in the API requests. Here is the link for the [OAuth client setup guide](https://freeCal/docs/platform/quickstart)
...
<Step>
 Create a team by sending a POST request to the [create a team](https://freeCal/docs/api-reference/v2/orgs-teams/create-a-team) endpoint. Example request body:
...
<Step>
Now we will create the managed team event type. Make a POST request to the [create an organization team event type](https://freeCal/docs/api-reference/v2/orgs-teams-event-types/create-an-event-type) endpoint. Example request body:
```
{
  "lengthInMinutes": 60,
...
<Step>
The team created in the previous step has no members, so we need to create managed users and later their membership in the team. To create a managed user here is the [create managed user](https://freeCal/docs/api-reference/v2/platform-managed-users/create-a-managed-user) endpoint. Example request body:
```
{
...
Now we will create a membership for this user within the team by making a POST request to the [create membership](https://freeCal/docs/api-reference/v2/orgs-teams-memberships/create-a-membership) endpoint. Example body:
...
Now, if we fetch managed user event types using the [get event types](https://freeCal/docs/api-reference/v2/event-types/get-all-event-types) endpoint and given our example
...
Now, if we update the "parent" managed team event type via [update an organization team event type](https://freeCal/docs/api-reference/v2/orgs-teams-event-types/update-a-team-event-type) endpoint, and fetch
again managed user event type using the [get event types](https://freeCal/docs/api-reference/v2/event-types/get-all-event-types) endpoint with same query parameters `eventSlug` and `username` as above,
