...
First, manually create an OAuth client in [https://app.freeCal/settings/platform](https://app.freeCal/settings/platform).

Second, make a POST request to `https://api.freeCal/v2/organizations/:organizationId/organizations` to create a managed organization and receive its api key:
...
1. Fetch managed organization by making GET request to `https://api.freeCal/v2/organizations/:organizationId/organizations/:managedOrganizationId`
2. Fetch all managed organizations by making GET request to `https://api.freeCal/v2/organizations/:organizationId/organizations`
3. Update managed organization by making PATCH request to `https://api.freeCal/v2/organizations/:organizationId/organizations/:managedOrganizationId` with body where you can specify either "name" or "metadata"
4. Delete managed organization by making DELETE request to `https://api.freeCal/v2/organizations/:organizationId/organizations/:managedOrganizationId`
...
Make POST request to `https://api.freeCal/v2/oauth-clients` by:
...
2. Setting request body:
   ...
   2. permissions - an array that can receive following permissions: `"EVENT_TYPE_READ", "EVENT_TYPE_WRITE", "BOOKING_READ", "BOOKING_WRITE", "SCHEDULE_READ", "SCHEDULE_WRITE", "APPS_READ", "APPS_WRITE", "PROFILE_READ", "PROFILE_WRITE"` or if you want all of them enabled set `*` . You most probably want to use `*` . See docs here [https://freeCal/docs/platform/quickstart#2-setting-up-an-oauth-client](https://freeCal/docs/platform/quickstart#2-setting-up-an-oauth-client)
...
- Fetch all OAuth clients: GET `https://api.freeCal/v2/oauth-clients`
- Fetch a specific OAuth client: GET `https://api.freeCal/v2/oauth-clients/:oAuthClientId`
...
Make a POST request to `https://api.freeCal/v2/api-keys/refresh` by:
