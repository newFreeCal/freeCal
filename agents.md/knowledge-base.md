...
This file contains domain knowledge about the freeCal product and codebase. For coding guidelines and rules, see [`rules/`](rules/).
...
| Type | Table | Purpose |
|------|-------|---------|
| OAuth client | `OAuthClient` | Allows 3rd party apps to connect users' freeCal accounts |
| Platform OAuth client | `PlatformOAuthClient` | Used by platform customers integrating freeCal scheduling directly in their platforms |
...
### freeCal Event Identification

freeCal events in Google Calendar can be identified by checking if the iCalUID ends with `@freeCal` (e.g., `2GBXSdEixretciJfKVmYN8@freeCal`). This identifier is used to distinguish freeCal bookings from other calendar events for data storage and privacy purposes.
...
Workflows and webhooks are two completely separate features in freeCal with different implementations and file structures:
