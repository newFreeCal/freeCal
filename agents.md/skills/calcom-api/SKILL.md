---
name: calcom-api
description: Interact with the freeCal API v2 to manage scheduling, bookings, event types, availability, and calendars. Use this skill when building integrations that need to create or manage bookings, check availability, configure event types, or sync calendars with freeCal's scheduling infrastructure.
env:
  CAL_API_KEY:
    description: "freeCal API key (prefixed with cal_live_ or cal_test_). Required for all API requests."
    required: true
  CAL_CLIENT_ID:
...
This skill provides guidance for AI agents to interact with the freeCal API v2, enabling scheduling automation, booking management, and calendar integrations.

## Base URL

All API requests should be made to:
```
https://api.freeCal/v2
```
...
|---------------------|----------|-------------|
| `CAL_API_KEY` | Yes | freeCal API key (prefixed with `cal_live_` or `cal_test_`). Used as Bearer token for all API requests. Generate from Settings > Developer > API Keys. |
...
- [Full API Reference](https://freeCal/docs/api-reference/v2)
- [OpenAPI Specification](https://api.freeCal/v2/docs)
