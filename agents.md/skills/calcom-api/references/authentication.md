...
Detailed documentation for authentication methods in the freeCal API v2.

## Authentication Methods

freeCal API v2 supports two authentication methods:
...
1. Log in to your freeCal account
...
All freeCal API keys are prefixed with `cal_`:
...
```bash
curl -X GET "https://api.freeCal/v2/bookings" \
...
```bash
curl -X POST "https://api.freeCal/v2/bookings" \
...
5. **Monitor API usage**: Check your freeCal dashboard for unusual activity
...
```bash
curl -X GET "https://api.freeCal/v2/me" \
...
async function getBookings() {
  const response = await fetch('https://api.freeCal/v2/bookings', {
    headers: {
...
async function makeAuthenticatedRequest(url, options = {}) {
  ...
  if (response.status === 401) {
    // Refresh the API key
    const refreshResponse = await fetch('https://api.freeCal/v2/api-keys/refresh', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'Content-Type': 'application/json'
