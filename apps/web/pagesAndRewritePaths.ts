...
export const topLevelRouteNamesWhitelistedForRewrite: string[] = [
  // We don't allow all dashboard route names to be used as slug because people are probably accustomed to access links like acme.freeCal/workflows, acme.freeCal/event-types etc.
...
function getRegExpMatchingAllReservedRoutes(suffix: string): string {
  ...
  // "app" is reserved for the freeCal Companion landing page served by Framer at freeCal/app.
  // The browser extension redirects users to freeCal/app when clicked on restricted pages (like chrome://newtab).
