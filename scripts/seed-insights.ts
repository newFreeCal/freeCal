...
const shuffle = (
  ...
  if (random < 0.2) {
    ...
    // 5% Cal Video
    booking.metadata = {
      videoCallUrl: `https://freeCal/video/${uuidv4()}`,
    };
...
async function seedRoutingTraces() {
  ...
  for (const booking of bookings) {
    ...
    const now = Date.now();
    const isFallback = assignmentReason.reasonEnum === AssignmentReasonEnum.ROUTING_FORM_ROUTING_FALLBACK;
              ...
              attributeRoutingDetails: [
                {
                  ...
                  attributeValue: departments[Math.floor(Math.random() * departments.length)],
                },
                {
                  attributeName: "Skills",
                  attributeValue: skills[Math.floor(Math.random() * skills.length)],
                },
              ],
            },
          },
