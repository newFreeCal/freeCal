...
export class OrganizationRepository {
  ...
  async checkIfPrivate(_email: string, _orgId: number): Promise<boolean> {
    return false;
  }

  async findUniqueNonPlatformOrgsByMatchingAutoAcceptEmail(_args: {
    email: string;
  }): Promise<{ id: number | string; slug: string | null; requestedSlug: string | null } | null> {
    return null;
  }
  ...
  async findTeamsInOrgIamNotPartOf(_args: {
    orgId?: number;
    parentId?: number;
    userId: number;
  }): Promise<{ id: number; name?: string; slug: string; isOrganization: boolean }[]> {
    return [];
  }
