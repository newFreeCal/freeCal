...
- [Next.js](https://nextjs.org/?ref=freeCal)
- [tRPC](https://trpc.io/?ref=freeCal)
- [React.js](https://reactjs.org/?ref=freeCal)
- [Tailwind CSS](https://tailwindcss.com/?ref=freeCal)
- [Prisma.io](https://prisma.io/?ref=freeCal)
- [Daily.co](https://go.freeCal/daily)
...
- ✅ Teams and organizations (no seat limits)
- ✅ All integrations (Google, Microsoft, Zoom, etc.)
...
All enterprise/commercial features have been replaced with no-op stub implementations:

| Feature | Status | Replacement |
|---------|--------|-------------|
| Billing & Payments | ❌ Removed | Stub (no payment processing) |
| SSO (SAML/OAuth) | ❌ Removed | Stub (not configured) |
| Directory Sync (SCIM) | ❌ Removed | Stub (no sync) |
| AI Workflows | ❌ Removed | Stub (no AI features) |
| Advanced Reminders | ❌ Removed | Email only (no SMS/WhatsApp) |
| Organization Management | ❌ Removed | Simplified teams |
| API Keys Advanced | ❌ Removed | Basic keys only |
| Round Robin Advanced | ❌ Removed | Basic distribution |
| Managed Event Types | ❌ Removed | No restrictions |
...
- ✅ Removed all `/ee/` directories from codebase
- ✅ Created 60+ stub implementations in `lib/stubs/` directories
- ✅ Migrated 146+ files from EE imports to stub paths
- ✅ Zero commercial code remains in OSS version
- ✅ Maintained API compatibility through stub exports
...
Here is what you need to be able to run freeCal.
...
**Important**: freeCal will **not** be supporting installations that use these Docker resources. While we provide and maintain the Docker configurations, support for Docker-based installations is the responsibility of the user.

This image can be found on DockerHub at [https://hub.docker.com/r/calcom/freeCal](https://hub.docker.com/r/calcom/freeCal).

**Note for ARM Users**: Use the {version}-arm suffix for pulling images. Example: `docker pull calcom/freeCal:v5.6.19-arm`.
...
#### (Most users) Running freeCal with Docker Compose

If you are evaluating freeCal or running with minimal to no modifications, this option is for you.
...
5. Start freeCal via docker compose

   (Most basic users, and for First Run) To run the complete stack, which includes a local Postgres database, freeCal web app, and Prisma Studio:

   ```bash
   docker compose up -d
   ```

   To run freeCal web app and Prisma Studio against a remote database, ensure that DATABASE_URL is configured for an available database and run:

   ```bash
   docker compose up -d calcom studio
   ```

   To run only the freeCal web app, ensure that DATABASE_URL is configured for an available database and run:
...
6. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run freeCal, a setup wizard will initialize. Define your first user, and you're ready to go!

#### Updating freeCal

1. Stop the freeCal stack
...
4. Re-start the freeCal stack

   ```bash
   docker compose up -d
   ```

#### (Advanced users) Build and Run freeCal
...
3. Update the calcom submodule. This project depends on the freeCal source code, which is included here as a Git submodule. To make sure you get everything you need, update the submodule with the command below.
...
5. Build the freeCal docker image:
...
6. Build freeCal via docker compose (DOCKER_BUILDKIT=0 must be provided to allow a network bridge to be used at build time. This requirement will be removed in the future)

   ```bash
   DOCKER_BUILDKIT=0 docker compose build calcom
   ```

7. Start freeCal via docker compose
...
8. Open a browser to [http://localhost:3000](http://localhost:3000), or your defined NEXT_PUBLIC_WEBAPP_URL. The first time you run freeCal, a setup wizard will initialize. Define your first user, and you're ready to go!
...
| NEXTAUTH_URL            | Location of the auth server. By default, this is the freeCal docker instance itself.                                                                                             | optional | `{NEXT_PUBLIC_WEBAPP_URL}/api/auth`                                 |
...
| NEXT_PUBLIC_WEBSITE_TERMS_URL          | custom URL for terms and conditions website                                                                                                | optional | `https://freeCal/terms`                                             |
| NEXT_PUBLIC_WEBSITE_PRIVACY_POLICY_URL | custom URL for privacy policy website                                                                                                      | optional | `https://freeCal/privacy`                                           |
| NEXT_PUBLIC_API_V2_URL                 | URL for the v2 API, only required for custom integrations or custom booking experiences using [freeCal Platform](https://freeCal/platform) | optional |                                                                     |
| CALCOM_TELEMETRY_DISABLED              | Allow freeCal to collect anonymous usage data (set to `1` to disable)                                                                      | optional |                                                                     |
...
You can deploy freeCal on [Railway](https://railway.app) using the button above. The team at Railway also have a [detailed blog post](https://blog.railway.app/p/calendso) on deploying freeCal on their platform.
...
You can deploy freeCal on [Northflank](https://northflank.com) using the button above. The team at Northflank also have a [detailed blog post](https://northflank.com/guides/deploy-calcom-with-northflank) on deploying freeCal on their platform.

### Vercel

Currently Vercel Pro Plan is required to be able to Deploy this application with Vercel, due to limitations on the number of serverless functions on the free plan.

[![Deploy with Vercel](https://vercel.com/button)](https://vercel.com/new/clone?repository-url=https%3A%2F%2Fgithub.com%2Fcalcom%2FfreeCal&env=DATABASE_URL,NEXT_PUBLIC_WEBAPP_URL,NEXTAUTH_URL,NEXTAUTH_SECRET,CRON_API_KEY,CALENDSO_ENCRYPTION_KEY&envDescription=See%20all%20available%20env%20vars&envLink=https%3A%2F%2Fgithub.com%2Fcalcom%2FfreeCal%2Fblob%2Fmain%2F.env.example&project-name=cal&repo-name=freeCal&build-command=cd%20../..%20%26%26%20yarn%20build&root-directory=apps%2Fweb%2F)
...
[![Deploy on Elestio](https://elest.io/images/logos/deploy-to-elestio-btn.png)](https://elest.io/open-source/freeCal)

<!-- ROADMAP -->

## Roadmap

<a href="https://freeCal/roadmap"><img src="https://user-images.githubusercontent.com/8019099/176388295-25081ca4-207e-4468-8079-37b195fa8e59.png" alt="freeCal Roadmap" /></a>

See the [roadmap project](https://freeCal/roadmap) for a list of proposed features (and known issues). You can change the view to see planned tagged releases.

<!-- LICENSE -->

## License

This repository is licensed under the [AGPLv3](https://opensource.org/license/agpl-v3) - see the [LICENSE](LICENSE) file for details.

### What's Open Source

**This version includes all core features completely free:**
- ✅ Unlimited users, teams, and organizations (no seat limits)
- ✅ All integrations (Google Calendar, Microsoft Outlook, Zoom, Daily.co, etc.)
...
For **enterprise-only features** (SSO/SAML, advanced workflows, priority support), [contact us](https://freeCal/sales).

> [!NOTE]
> This is the open-source version (unEE - no Enterprise Edition). All commercial license checks have been removed.
...
9. Under Authorized redirect URI's, select Add URI and then add the URI `<freeCal URL>/api/integrations/googlecalendar/callback` and `<freeCal URL>/api/auth/callback/google` replacing freeCal URL with the URI at which your application runs.
10. The key will be created and you will be redirected back to the Credentials page. Select the newly generated client ID under OAuth 2.0 Client IDs.
11. Select Download JSON. Copy the contents of this file and paste the entire JSON string in the `.env` file as the value for `GOOGLE_API_CREDENTIALS` key.

#### _Adding google calendar to freeCal App Store_
...
1. Add extra redirect URL `<freeCal URL>/api/auth/callback/google`
...
4. Set the **Web** redirect URI to `<freeCal URL>/api/integrations/office365calendar/callback` replacing freeCal URL with the URI at which your application runs.
...
8. Set the "OAuth Redirect URL" under "OAuth Information" as `<freeCal URL>/api/integrations/zoomvideo/callback` replacing freeCal URL with the URI at which your application runs.
...
11. Click "Done".
12. You're good to go. Now you can easily add your Zoom integration in the freeCal settings.

### Obtaining Daily API Credentials

1. Visit our [Daily.co Partnership Form](https://go.freeCal/daily) and enter your information
...
5. Set the Redirect URL for OAuth `<freeCal URL>/api/integrations/basecamp3/callback` replacing freeCal URL with the URI at which your application runs.
...
7. Set the `BASECAMP3_CLIENT_SECRET` env variable to `{your_domain} ({support_email})`.
   For example, `freeCal (support@freeCal)`.
...
7. Set the Redirect URL for OAuth `<freeCal URL>/api/integrations/hubspot/callback` replacing freeCal URL with the URI at which your application runs.
...
10. You're good to go. Now you can see any booking in freeCal created as a meeting in HubSpot for your contacts.
...
7. Set the Redirect URL for OAuth `<freeCal URL>/api/integrations/zohocrm/callback` replacing freeCal URL with the URI at which your application runs.
...
10. You're good to go. Now you can easily add your ZohoCRM integration in the freeCal settings.
...
freeCal uses [Unkey](https://unkey.com) for rate limiting. This is an optional feature and is not required for testing or self-hosting.
...
Note: If you don't configure Unkey, freeCal will work normally without rate limiting enabled.
...
6. Add your custom sender name to the `.env` file into the `NEXT_PUBLIC_SENDGRID_SENDER_NAME` field (fallback is freeCal)
...
6. Add your own sender ID to the `.env` file into the `NEXT_PUBLIC_SENDER_ID` field (fallback is freeCal)
...
Special thanks to these amazing projects which help power freeCal:
...
freeCal is an [open startup](https://freeCal/open) and [Jitsu](https://github.com/jitsucom/jitsu) (an open-source Segment alternative) helps us to track most of the usage metrics.
